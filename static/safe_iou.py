...
def compute_mIoU(gt_dir, pred_dir, devkit_dir=''):
    ...
    print(f'MIoU of main claases :{str(round(np.nanmean(mIoUs[:len(IMP_CLASSES)]) * 100, 2))} , Safe_MIoU iou of main classes:   {str(round(np.nanmean(SmIoUs[:len(IMP_CLASSES)]) * 100, 2))}')
    print('=========')
    return  mIoUs, SmIoUs 
